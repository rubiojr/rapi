...
type Index struct {
	m         sync.Mutex
	byType    [blob.NumBlobTypes]indexMap
	packs     restic.IDs
...
func (idx *Index) store(packIndex int, blob blob.Blob) {
...
func (idx *Index) Store(blob blob.PackedBlob) {
	idx.m.Lock()
	defer idx.m.Unlock()
...
// in the index
func (idx *Index) StorePack(id restic.ID, blobs []blob.Blob) {
	idx.m.Lock()
	defer idx.m.Unlock()
...
func (idx *Index) toPackedBlob(e *indexEntry, typ blob.BlobType) blob.PackedBlob {
	return blob.PackedBlob{
		Blob: blob.Blob{
			ID:     e.id,
			Type:   typ,
...
func (idx *Index) Lookup(id restic.ID, tpe blob.BlobType, blobs []blob.PackedBlob) []blob.PackedBlob {
	idx.m.Lock()
	defer idx.m.Unlock()
...
func (idx *Index) ListPack(id restic.ID) (list []blob.PackedBlob) {
	idx.m.Lock()
	defer idx.m.Unlock()
	...
	for typ := range idx.byType {
		...
		m.foreach(func(e *indexEntry) bool {
			if idx.packs[e.packIndex] == id {
				list = append(list, idx.toPackedBlob(e, blob.BlobType(typ)))
			}
			return true
		})
	}
...
func (idx *Index) Has(id restic.ID, tpe blob.BlobType) bool {
	idx.m.Lock()
	defer idx.m.Unlock()
...
// given id.
func (idx *Index) LookupSize(id restic.ID, tpe blob.BlobType) (plaintextLength uint, found bool) {
	idx.m.Lock()
	defer idx.m.Unlock()
...
func (idx *Index) Each(ctx context.Context) <-chan blob.PackedBlob {
	idx.m.Lock()

	ch := make(chan blob.PackedBlob)
	...
	go func() {
		...
		for typ := range idx.byType {
			...
			m.foreach(func(e *indexEntry) bool {
				...
				case <-ctx.Done():
					return false
				case ch <- idx.toPackedBlob(e, blob.BlobType(typ)):
					return true
				}
			})
		}
	...
	go func() {
		...
		for typ := range idx.byType {
			...
			for packID, pack := range byPack {
				...
				for _, e := range pack {
					result.blobs = append(result.blobs, idx.toPackedBlob(e, blob.BlobType(typ)).Blob)
				}
				select {
				case <-ctx.Done():
					return
...
func (idx *Index) Count(t blob.BlobType) (n uint) {
	...
	for typ := range idx2.byType {
		...
		hasIdenticalEntry := func(e2 *indexEntry) (found bool) {
			m.foreachWithID(e2.id, func(e *indexEntry) {
				b := idx.toPackedBlob(e, blob.BlobType(typ))
				b2 := idx2.toPackedBlob(e2, blob.BlobType(typ))
